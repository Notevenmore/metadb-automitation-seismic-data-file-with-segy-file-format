...
explorerSlider = DiscreteSlider(name='Explore Slice', value=100, max_height=50, options=list(np.arange(100, 750, 1)), start=100, end=750)
...
@pn.depends(explorerSlider.param.value, watch=True)
def seismicstackplot(explorer):
    print(explorerSlider.param.value)
    try:
        filenamevolin = './PROJECT/%s/%s/VOL/%s.vol' % (username,selectproject["value"], selectseismic["value"])
        fvolin = tables.open_file(filenamevolin, mode='r')
        sr = int(np.load('./PROJECT/%s/%s/TEMP/%s.samsr' % (username,selectproject["value"], selectseismic["value"]))[1]) / 1000
        ...
        # column = pn.Column(explorerSlider)
        # print(json_item(layout, "plot"))
        # file_obj = open("test.html", mode="wb")
        # slider_obj = pn.io.save(pn.Column(explorerSlider), file_obj, fmt="html")
        # print("P? ")
        # html_content = slider_obj.read().decode('utf-8')
        # print(html_content)
        layout = row(column(explorerSlider.get_root(), width=100), bokeh_fig)
        # print(explorerSlider.get_root())
...
def callSeismic():
    return seismicstackplot(explorerSlider.value)
