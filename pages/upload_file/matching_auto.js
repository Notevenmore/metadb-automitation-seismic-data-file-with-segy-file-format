...
const fetchAutoFill = (async (docId, pageNo) => {
  var requestOptions = {
    method: 'GET',
    redirect: 'follow'
  };
...
export default function MatchReview({ setTitle }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (files.length < 1) {
        ...
        for (let pageNo = 1; pageNo <= summaryResponse.body.page_count; pageNo++) {
          const autoFillResponse = await fetchAutoFill(docId, pageNo);
          if (autoFillResponse.status === "success") {
            if (pageNo === 1) {
              const { pairs } = autoFillResponse.body;
              setPairs(pairs);
            }
          }
        }
  ...
  const setValueForAllKey = ((key, value) => {
    setState((state) => {
      const index = state.findIndex(pair => pair.key === key);
      // const indexes = state.filter(pair => pair.key === key);
      const cpair = state.find(pair => pair.key == key);
      const newPair = { ...cpair, value };
      return [...state.slice(0, index), newPair, ...state.slice(index + 1)];
    });
  });
  ...
  const setPairs = ((pair) => {
    setState((state) => {
      ...
      for (let i = 0; i < state.length; i++) {
        if (keys.includes(state[i].key)) {
          indexes = indexes.concat(i);
        }
      }
      let newState = [...state];
      for (const index of indexes) {
        const cpair = newState[index];
        const newPair = { ...cpair, value: pair[newState[index].key] };
        newState = [...newState.slice(0, index), newPair, ...newState.slice(index + 1)]
      }
      return newState;
    });
  });
