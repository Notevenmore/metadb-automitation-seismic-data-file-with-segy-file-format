...
export default function UploadFilePage({config, setTitle, kkks_name}) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if(UplSettings.Method !== 'connect-local-directory') {
      ...
      router.push({
        pathname: `connect-to-local-directory/${UplSettings.DataType.toLowerCase().replaceAll(" ", "_")}`, 
      });
    }
  };
      ...
      <form
        ...
        <div className="w-4/5 max-w-[80%] flex items-center justify-center">
          <div
            ...
            {UplSettings.DataType.toLowerCase().includes('seismic') && !UplSettings.DataType.toLowerCase().includes("non") && UplSettings.FileFormat === "SGY/SEGY" && (
              <Button
                id="connect"
                title=""
                additional_styles={`h-full active:bg-gray-400/60 outline-none ${
                  UplSettings.Method === 'connect-local-directory' ? activeStyle : ''
                }`}
                onClick={e => {
                  e.preventDefault();
                  setUplSettings({...UplSettings, Method: 'connect-local-directory'});
                }}>
                <div className="flex space-x-2 min-w-max items-center p-2">
                  <LocalDir className="w-10 h-10" />
                  <section className="w-150p">
                    <h3 className="text-lg font-bold">Connect Local Directory</h3>
                    <p className="text-sm">
                      Load and Adjust SEGY or SGY data from local into an Excel format
                    </p>
                  </section>
                </div>
              </Button>
          ...
          <Button
            ...
            disabled={
              ...
              Object.values(UplSettings).some(x => {
                ...
                ? UplSettings.Method !== "connect-local-directory" || Object.values(UplSettings).some(x => {
                  return x === null || x === '';
                }) 
                  ? true 
                  : false
