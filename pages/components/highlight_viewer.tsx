...
export type ImageEditorContextType = {
  ...
  dispatch: Dispatch<ImageEditorAction>;
};
...
function imageEditorReducer(
  ...
  switch (action.actionType) {
    ...
    case ImageEditorActionType.SET_MODE_SELECT:
      if (state.selectorState.stateType !== SelectorStateType.COMPLETE)
        return state;
      return {
        ...state,
        selectorState: {
          stateType: SelectorStateType.FIRST_BOUND,
          bounds: [0, 0, 0, 0],
        },
    ...
    case SelectorActionType.SELECTING_BOUND_START:
      if (state.selectorState.stateType !== SelectorStateType.FIRST_BOUND)
        return state;
      return {
        ...state,
        selectorState: {
          ...state.selectorState,
          bounds: [...action.bounds, ...action.bounds],
        },
      };
    ...
    case SelectorActionType.SELECT_BOUND_START:
      ...
      return {
        ...
        selectorState: {
          ...state.selectorState,
          stateType: SelectorStateType.SECOND_BOUND,
        },
      };

    case SelectorActionType.SELECTING_BOUND_END:
      if (state.selectorState.stateType !== SelectorStateType.SECOND_BOUND)
        return state;
    ...
    case ImageEditorActionType.SET_MOUSE_POS_RELATIVE_TO_IMAGE:
      const relX = Math.ceil(
        (action.mousePos[0] - state.translate[0]) / state.scale,
      );
      const relY = Math.ceil(
        (action.mousePos[1] - state.translate[1]) / state.scale,
      );
      return {
        ...state,
...
export const ImageEditorProvider = ({
  children,
  boundsObserver,
}: PropsWithChildren<ImageEditorProviderProps>) => {
...
export const useTranslateDraggable = (
  ...
  const {
    state: {translate, selectedNavbarButton},
    dispatch,
  } = useContext(context);
...
export const useBoundingBox = (context: Context<ImageEditorContextType>) => {
  const {state, dispatch} = useContext(context);
  const {selectorState, mousePositionRelativeToImage} = state;
  const mouseDown = useMouseDown();
  ...
  useEffect(() => {
    ...
    const startDrawing =
      selectorState.stateType === SelectorStateType.FIRST_BOUND;
    ...
    const newButtonConfig: NavbarButtonConfig[] = [
      {
        ...
        imgSrc: '/highlight_viewer/all-directions_32.png',
        onClick: () =>
          dispatch({actionType: ImageEditorActionType.SET_MODE_MOVE}),
      },
      {
        id: '2',
        selected: selectedNavbarButton === NavbarButton.SELECT_BUTTON,
        imgSrc: '/highlight_viewer/select_32.png',
        onClick: () =>
          dispatch({actionType: ImageEditorActionType.SET_MODE_SELECT}),
      },
    ];
