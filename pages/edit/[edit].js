...
const DocEditor = ({workspace_name, config}) => {
  ...
  const put_workspace = async (form_type, afe, workspace_data) => {
    const data = await fetch(
      `http://localhost:9090/api/v1/${form_type}-workspace-afe/${afe}`,
      {
        method: 'PUT',
        headers: {
  ...
  const post_workspace = async (form_type, workspace_data) => {
    const data = await fetch(
      `http://localhost:9090/api/v1/${form_type}-workspace-afe`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const upload_afeguid_new = async (form_type, afe, guid) => {
    const data = await fetch(
      `http://localhost:9090/api/v1/${form_type}-workspace`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const delete_data = async (form_type, data_id, new_data) => {
    const data = await fetch(
      `http://localhost:9090/api/v1/${form_type}/${data_id}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    const init_data = async () => {
      try {
        ...
        if (router?.query?.form_type === 'printed_well_report') {
          ...
          workspace_data_array =
            JSON.parse(localStorage.getItem('bibliography')) || bibliography;
        }
        ...
        workspace_data_array.some(workspace_data => {
          ...
          if (
            workspace_name_space.toLowerCase() ===
            workspace_data.Name.toLowerCase()
          ) {
  ...
  const saveDocument = async e => {
    ...
    try {
      ...
      const spreadsheet_data = await fetch(
        `${config.services.sheets}/getRows`,
        {
          method: 'POST',
          headers: {
            'Content-type': 'application/json',
      ...
      for (
        let idx_row = 1;
        idx_row < spreadsheet_data.response.length;
        idx_row++
      ) {
        let row = {};
        console.log(idx_row);
        spreadsheet_data.response[0].forEach((header, idx_col) => {
          row[header.toLowerCase()] =
            spreadsheet_data?.response[idx_row][idx_col] || '';
        });
        final.push(row);
      ...
      const {data_type, afe_number, ...workspace_data_put} =
        Object.assign(workspace_data_post);
      ...
      if (workspace_name === 'new_document') {
        ...
        if (router.query.form_type === 'printed_well_report') {
          ...
          final.forEach(row => {
            post_data('bibliography', row).catch(err =>
              put_data('bibliography', row.ppdm_guid, row),
            );
          });
          post_workspace('bibliography', workspace_data_post).catch(err =>
            put_workspace('bibliography', workspace_name, workspace_data_put),
          );
          ppdm_guid_array.forEach(ppdm_guid => {
            upload_afeguid_new(
              'bibliography',
              parseInt(workspaceData.AFE),
              ppdm_guid,
            );
          });
        }
            ...
            ? [
                [
                  ...
                  <Input
                    ...
                    type={'text'}
                    defaultValue={
                      workspaceData?.KKS || 'Geodwipa Teknika Nusantara'
                    }
                    onChange={handleWorkspaceChange}
                  />,
                ],
                ...
                [
                  ...
                  <Input
                    ...
                    dropdown_items={submission_types}
                    onChange={handleWorkspaceChange}
                  />,
                ],
                [
                  ...
                  <Input
                    name="AFE"
                    type={'number'}
                    defaultValue={String(workspaceData?.AFE) || '1'}
                    disabled
                  />,
                ],
                [
