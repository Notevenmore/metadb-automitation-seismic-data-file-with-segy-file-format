import { AddDraggableAction, AddDroppableAction, ClearAllDragItemCanHoldAction, ClearAllDragItemHoldingAction, ClearAllDropItemCanDropAction, DraggableAct, DraggableAction, DraggableItemData, DraggableState, DroppableItemData, EMPTY_DRAGGABLE_ITEM_DATA, EMPTY_DROPPABLE_ITEM_DATA, SetDragItemCanHoldAction, SetDragItemHoldingAction, SetDraggablePositionAction, SetDroppableCanDropAction, SetDroppableDataAction, SetMouseDownAction, SetMouseMousePositionAction, Tuple2 } from "./types";

export function draggableReducer(state: DraggableState, action: DraggableAction): DraggableState {
    console.log(state);
    switch (action.act) {
        case DraggableAct.SET_DRAGGABLE_POSITION: { // private
        ...
        } case DraggableAct.SET_DRAG_ITEM_HOLDING: { // private
            return actor.reduceSetDragItemHolding(state, action);
        } case DraggableAct.SET_DRAG_ITEM_CAN_HOLD: { // public
        ...
        } case DraggableAct.CLEAR_ALL_DRAG_ITEM_CAN_HOLD: { // private
...
const actor = {
    ...
    reduceWhenMouseDown: function (
        ...
        const clearedHoldingState = draggableReducer(clearAllCanDropState, { act: clearHolding });
        const res = Mutate.from(clearedHoldingState).setDraggableItemDataById(newItem.id, newItem);
        return res;
    },

    reduceWhenMouseUp: function (
        state: DraggableState
    ): DraggableState {
        ...
        if (dropItem) {
            ...
            newState = Mutate.from(newState).setDroppableItemDataById(newDropItem.id, newDropItem);
        }
...
class Mutate {
    ...
    setDraggableItemDataById(itemId: number, item: DraggableItemData): DraggableState{
        const state = this.state;
        const items = state.draggableItems;
        ...
        return {
            ...state,
            draggableItems: [...items.slice(0, index), {...item, id: itemId}, ...items.slice(index + 1)]
        };
    }

    setDroppableItemDataById(itemId: number, item: DroppableItemData): DraggableState {
        const state = this.state;
        const items = state.droppableItems;
        ...
        return {
            ...state,
            droppableItems: [...items.slice(0, index), { ...item, id: itemId}, ...items.slice(index + 1)]
        }
    }
