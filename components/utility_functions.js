        ...
        for (const datatype_record_id of data) {
            ...
            Object.keys(data_details[0]).forEach(key => {
                if (key.toLowerCase().includes("date") && data_details[0][key]) {
                    ...
                    data_details[0][key] = `${new Intl.DateTimeFormat('ja-JP').format(date).replace(/\//g, "-")}`
                }
            });
...
const saveDocument = async (e, router, config, spreadsheetId, workspaceData, setMessage) => {
    if (e) {
        e.preventDefault()
    }
            ...
            spreadsheet_header.response.forEach((header, idx_col) => {
                ...
                try {
                    if (header.toLowerCase().includes("date")) {
                        let old_date = old_data.data_content[idx_row][header.toLowerCase()] || old_data.data_content[idx_row][header] || null
                        if (old_date) {
                            old_date = new Date(old_date)
                            old_date = `${old_date.getDate().toString().padStart(2, "0")}-${(old_date.getMonth() + 1).toString().padStart(2, "0")}-${old_date.getFullYear()}`
                ...
                if (header.toLowerCase().includes("date") && row[header.toLowerCase()]) {
                    ...
                    try {
                        ...
                        const input = spreadsheet_data?.response[idx_row][idx_col].replace(/[^\x00-\x7F]/g, "")
                        if (input.includes("-")) {
                            parts = input.split("-");
...
const downloadWorkspace = async (router, config, spreadsheetId, workspaceData, setMessage) => {
...
module.exports = {
    init_data, saveDocument, downloadWorkspace
}